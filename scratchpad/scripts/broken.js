...
AutoSuggestContainer.prototype = {
	...
	keyupHandler: function (event) {
		...
		if (!this.enterClicked) {
			...
			if (trigger) {
				...
				if (node.data) {
					index = node.data.trim().length-trigger.trim().length;
					try {

						duplicateRange.setStart(node,index);
						var str = duplicateRange+"";
						//console.log(str.length,trigger.length,node.data.length,index)
						console.log(str.trim().length,trigger.trim().length,node.data.trim().length,index)

						var anotherNode = node.splitText(index)
	...
	clicked: function (id,text) {
		var selection = document.getSelection(),
			...
			currentTextNode = range.commonAncestorContainer
		if (this.editableElement == currentTextNode || this.editableElement.contains(currentTextNode)) {
			range.commonAncestorContainer.parentNode.insertBefore(newNode,range.commonAncestorContainer)
			var cursor = document.createTextNode("\u200b");
			range.commonAncestorContainer.parentNode.replaceChild(cursor,range.commonAncestorContainer);
