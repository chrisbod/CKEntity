	...
	return {
		...
		onShow: function () {
			...
			if (data) {
				...
				if (dialogSizes[wrapper.firstChild.id]) {
					var metrics = dialogSizes[wrapper.firstChild.id]

					this.resize(metrics.size.width-22,metrics.size.height-88);
					this.move(metrics.position.x, metrics.position.y)
				} else {

					var dialogSize = this.parts.dialog.$.getBoundingClientRect(),
						windowSize = {top: 0,left:0,width:window.innerWidth,height:window.innerHeight};
						windowSize.bottom = windowSize.height
						windowSize.right = windowSize.width

					if (dialogSize.bottom > windowSize.bottom) {
						console.log(dialogSize.height + (windowSize.bottom - dialogSize.bottom))
						this.resize(dialogSize.width,dialogSize.height + (windowSize.bottom - dialogSize.bottom));
		...
		onHide: function () {
			var wrapper = document.getElementById("knockoutWrapper");
			dialogSizes[data.element.id] = {
				size:this.getSize(),
				position: this.getPosition()
			}
			console.log(dialogSizes[data.element.id])
			wrapper.style.height = "";
			if (restoreOpacity) {
				document.querySelector("div.cke_dialog_background_cover").style.opacity = "0.5";
			}
